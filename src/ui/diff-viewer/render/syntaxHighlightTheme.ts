...
async function buildCustomSyntaxTheme(
  ...
  const scopeSettings = Object.entries(scopeOverrides).map(
    ([scope, foreground]) => ({
      scope,
      settings: { foreground },
    }),
...
export function syntaxHighlightThemeName(
  theme: AppTheme | AppTheme["appearance"],
) {
...
export function ensureSyntaxHighlightThemeRegistered(
  theme: AppTheme | AppTheme["appearance"],
) {
