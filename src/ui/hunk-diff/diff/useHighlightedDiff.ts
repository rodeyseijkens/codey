...
import { type HighlightedDiffCode, loadHighlightedDiff } from "./pierre";
...
const SHARED_HIGHLIGHT_PROMISES = new Map<
  string,
...
export function useHighlightedDiff({
  ...
  const [highlighted, setHighlighted] = useState<HighlightedDiffCode | null>(
    null,
  );
  const [highlightedCacheKey, setHighlightedCacheKey] = useState<string | null>(
    null,
  );
  ...
  useLayoutEffect(() => {
    ...
    ensureHighlightedDiffLoaded(file, theme, appearanceCacheKey).then(
      (nextHighlighted) => {
        if (cancelled) {
          return;
        }
