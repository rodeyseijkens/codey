...
// biome-ignore lint/suspicious/noControlCharactersInRegex: intentional control character matching
const controlNewlineTabRegex = /[\x00-\x08\x0b-\x1f\x7f-\x9f]/g;
// biome-ignore lint/suspicious/noControlCharactersInRegex: intentional control character matching
const controlTabRegex = /[\x00-\x09\x0b-\x1f\x7f-\x9f]/g;
// biome-ignore lint/suspicious/noControlCharactersInRegex: intentional control character matching
const controlNewlineRegex = /[\x00-\x08\x0a-\x1f\x7f-\x9f]/g;
// biome-ignore lint/suspicious/noControlCharactersInRegex: intentional control character matching
const controlAllRegex = /[\x00-\x1f\x7f-\x9f]/g;
// biome-ignore lint/suspicious/noControlCharactersInRegex: intentional ANSI escape matching
const ansiSgrRegex = /^\x1b\[[0-9;:]*m$/;
...
export function sanitizeTerminalText(
  ...
  let controlCharacters: RegExp;
  if (preserveNewlines) {
    controlCharacters = preserveTabs ? controlNewlineTabRegex : controlTabRegex;
  } else {
    controlCharacters = preserveTabs ? controlNewlineRegex : controlAllRegex;
  }
  const preservedStyles: string[] = [];
  const preserveStyle = (sequence: string) => {
    if (!(preserveAnsiStyle && ansiSgrRegex.test(sequence))) {
      return "";
    }
