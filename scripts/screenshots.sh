...
for tape in "$SCRIPT_DIR"/screenshots/*.tape; do
  ...
  rm -rf "$DEMO_DIR" "$WORK_DIR"/harbor-remote.git "$WORK_DIR"/harbor-diverge
  "$SCRIPT_DIR/screenshots/demo-repo.sh" "$DEMO_DIR" "${demo_flags[@]}"

  printf 'theme = "%s"\n' "$theme" > "$HOME_DIR/.config/codey/config.toml"
  if [ -n "$mode" ]; then
    printf 'mode = "%s"\n' "$mode" >> "$HOME_DIR/.config/codey/config.toml"
  fi
  echo "rendering $name (theme: $theme${mode:+, mode: $mode}${demo:+, demo: $demo})"
  (
    cd "$WORK_DIR"
